...
def validate_dataset(uploaded_df: pd.DataFrame) -> tuple[bool, str, str]:
    ...
    """
    # Define the mandatory features (X) and their expected types
    expected_features = {
        'customerID': 'str',
        'gender': 'str',
        'SeniorCitizen': 'int64',
        ...
        'TotalCharges': 'float64'   # Handled cleanly as float now
    }
    
    # Define the optional target label (y)
    target_col = "Churn"
    ...
    if uploaded_base_cols != mandatory_cols:
        ...
        msg += f"The first {len(mandatory_cols)} columns must exactly match the template features."
        return False, "INVALID", msg

    # ----------------------------------------------------
    # STEP : Check Data Types & Normalize 'str' Key
    # ----------------------------------------------------
    for col, expected_dtype in expected_features.items():
        actual_dtype = str(uploaded_df[col].dtype)
        
        # Safely treat 'str', 'object', and 'string' as the same category
    ...
    # ----------------------------------------------------
    # STEP: Check for Null Values & Empty Spaces
    # ----------------------------------------------------
    for col in mandatory_cols:
        # Check for standard pandas NaN/None values across all column types
        null_count = uploaded_df[col].isnull().sum()
        
        # Check for hidden empty strings ONLY if it is truly parsed as a text column
        actual_dtype = str(uploaded_df[col].dtype)
        if actual_dtype in ["object", "string" , "str"]:
            # Count fields that are completely empty or just contain blank spaces
            empty_space_count = (uploaded_df[col].astype(str).str.strip() == '').sum()
            null_count += empty_space_count

        # Since TotalCharges is now a float, any original text blanks were converted to NaN in app.py
        # We check if those NaNs correspond to invalid rows (where tenure is greater than 0)
        if col == 'TotalCharges' and null_count > 0:
            invalid_blanks = uploaded_df[uploaded_df[col].isnull() & (uploaded_df['tenure'] != 0)]
            ...
            continue # Safely skip further errors if they are allowed new customer blanks (tenure == 0)

        if null_count > 0:
            return False, "INVALID", f"Column '{col}' contains {null_count} missing (NaN or empty) values. Please clean your dataset."

    # ----------------------------------------------------
    # STEP: Automate Target (y) Detection
    # ----------------------------------------------------
    if len(uploaded_cols) == len(mandatory_cols):
    ...
    elif len(uploaded_cols) == len(mandatory_cols) + 1 and uploaded_cols[target_col] == target_col:
        actual_target_dtype = str(uploaded_df[target_col].dtype)
        
        # Check target nulls dynamically based on type
        ...
        # FIX: Dynamically evaluate using your expected_target_dtype variable
        valid_text_types = ["object", "string", "str"]
            
        # Fallback for exact matching if you ever change target to an int or float
        if expected_target_dtype == actual_target_dtype:
